...
class DUNETLoss(nn.Module):
    def __init__(self, n, hard_mining = 0, norm = False):
        super(DUNETLoss, self).__init__()
...
class Net(nn.Module):
    def __init__(self, input_size, block, fwd_out, num_fwd, back_out, num_back, n, hard_mining = 0, loss_norm = False):
        ...
        self.loss = DUNETLoss(n, hard_mining, loss_norm)
...
class DUNet():
    def __init__(self, device=None, res_output=True, ckpt=None) -> None:
        input_size = [384, 384]
        block = Conv
        ...
        self.model = Denoise(input_size[0], input_size[1], block, 3, fwd_out, num_fwd, back_out, num_back)
        ...
        for epoch in range(self.curr_epoch, max_epoch):
            ...
            for i, data in enumerate(data_loader, 0):
                ...
                if self.res_output:
                    ...
                    # img = toimg((inputs - inputs_noise)[0])
                    # img.save('/data/guohaoxun/workspace/atk/AdvDefenseFramework/tmp/0res.png')
                else:
...
if __name__ == '__main__':
    ...
    summary(denoise, input_size=(3,384,384), batch_size=1, device='cpu')
