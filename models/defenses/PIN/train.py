...
#pdb.set_trace()
config = Config()
logger = get_logger(os.path.join('logs',str(datetime.now())[:16].replace(' ', '_')+'.log'))
...
for epoch in range(EPOCHES):
    ...
    if last_epoch_time==0:
        ETA = (EPOCHES - epoch - 1) * epoch_time
    else:
        ETA = (EPOCHES - epoch - 1) * (epoch_time + last_epoch_time)/2
