...
class MCLoss_mean_plus_cons(nn.Module):
    """v1.1
    likehood use mean instead of sum, and add a constant such as 1 to make likehood above 0
    """
    def __init__(self, lamb, eigen_path, device, num_sample=30):
        super(MCLoss, self).__init__()
        self.eigen_data = torch.load(eigen_path)
    ...
    def Loss(self, samples, image, image_noise):
        #pdb.set_trace()
        index_samples = np.where(samples.cpu().numpy()==1)
        if index_samples[1].shape[0]==0:
            return 0
        num_eig = index_samples[1].max() + 1
        samples = samples[:,:num_eig]
        eig_face = self.eigen_data['eig_vec'][:,:num_eig].to(self.device)
        avg_face = self.eigen_data['avg_face'].to(self.device)
...
class MCLoss_rescale(nn.Module):
    """v1.2
    rescale the prob
    """
    def __init__(self, lamb, eigen_path, device, num_sample=30):
    ...
    def LogLikehood(self, pca_act, samples):
        ...
        dims = float(samples.size(1))
        samples = samples.float()
        probability = ((1-samples) - pca_act).abs()+1
        Likehood = torch.log(probability[:,0])
        for i in range(samples.size(1)-1):
            Likehood = Likehood + torch.log(probability[:,i+1])
        #pdb.set_trace()
        #LogLikehood = torch.log(Likehood)
        Likehood = Likehood/dims
        return Likehood
...
class MCLoss(nn.Module):
    ...
    def log_sign(self, input:torch.Tensor):
        return torch.sign(input)*torch.log(input.abs()+1)

    def LogLikehood(self, pca_act, samples):
        pca_act = pca_act.unsqueeze(0).expand(self.num_sample, samples.size(1))
        dims = float(samples.size(1))
        samples = samples.float()
        probability = -((1-samples) - pca_act)
        Likehood = self.log_sign(probability[:,0])
        for i in range(samples.size(1)-1):
            Likehood = Likehood + self.log_sign(probability[:,i+1])
        #pdb.set_trace()
        #LogLikehood = torch.log(Likehood)
        Likehood = Likehood/dims
        return Likehood
